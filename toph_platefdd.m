...
function toph_platefdd(nelx,nely,volfrac,penal,rmin,ft,method)
% INITIALIZE
...
% ---------------------------------------------------------------
% nested helper: calculate heat conduction objective for a given density field
function obj = heat_objective(xPhys_input)
    L   = 0.10;                       % plate size  [m]
    a   = 0.015;                      % half die-width
    q0  = 1e6;                        % heat flux   [W/m²]
    k_s = 1;                          % normalise solid conductivity to 1
    [~,~,U_temp] = FE(nelx,nely,xPhys_input,penal,q0,a,L,k_s);
    
    % Calculate objective using the same method as in main loop
    [KE_temp] = lk;
    obj = 0.;
    for ely = 1:nely
        for elx = 1:nelx
            n1 = (nely+1)*(elx-1)+ely; 
            n2 = (nely+1)* elx   +ely;
            Ue = U_temp([n1; n2; n2+1; n1+1],1);
            val = xPhys_input(ely,elx);                    
            kappa = 0.001 + 0.999*val^penal;      % same factor
            obj = obj + kappa * Ue' * KE_temp * Ue;
        end
    end
end
...
switch method
  ...
  %======================================================================
    ...
    % ---- objective with perturbed design ------------------------
         c_pert = heat_objective(xPhys_pert);
