...
function top88_ex4(nelx,nely,volfrac,penal,rmin,ft)
...
%% PASSIVE ELEMENT MASKING
% initialise: 0 = free to change, 1 = forced VOID, 2 = forced SOLID
passive = zeros(nely,nelx);

% ====== rectangular hole dimensions (edit to taste) ======
holeW  = round(0.50*nelx);        % width  = 30 % of span
holeH  = round(0.30*nely);        % height = 20 % of depth

% centred rectangle
x1 = round((nelx-holeW)/2) + 1;   % +1 because MATLAB indices start at 1
x2 = x1 + holeW - 1;
y1 = round((nely-holeH)/2) + 1;
y2 = y1 + holeH - 1;

% tag those elements as passive void (value = 1)
passive(y1:y2, x1:x2) = 1;
...
while change > 0.01
  ...
  while (l2-l1)/(l1+l2) > 1e-3
    ...
    if ft == 1
      xPhys = xnew;
      xPhys(passive==1) = 0;   % force the rectangular region to minimum density
      % xPhys(passive==2) = 1; % (use this line only if you ever add solid regions)
    elseif ft == 2
      xPhys(:) = (H*xnew(:))./Hs;
      xPhys(passive==1) = 0;   % force the rectangular region to minimum density
      % xPhys(passive==2) = 1; % (use this line only if you ever add solid regions)
